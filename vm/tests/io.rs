...
fn stdio() -> Result<(), std::io::Error> {
  test_file_with_stdout(
    "std_lib/io/stdio/stdout/write.tube",
    vec!["expected 1 expected 2"],
    VmExit::Ok,
  )?;

  test_file_with_stdout(
    "std_lib/io/stdio/stdout/writeln.tube",
    vec!["expected 1", "expected 2"],
    VmExit::Ok,
  )?;

  test_file_with_stderr(
    "std_lib/io/stdio/stderr/write.tube",
    vec!["expected 1 expected 2"],
    VmExit::Ok,
  )?;

  test_file_with_stderr(
    "std_lib/io/stdio/stderr/writeln.tube",
    vec!["expected 1", "expected 2"],
    VmExit::Ok,
  )?;

  test_file_with_stdin(
    "std_lib/io/stdio/stdin/read.tube",
    "expected".to_string(),
    VmExit::Ok,
  )?;

  test_file_with_stdin_lines(
    "std_lib/io/stdio/stdin/readline.tube",
...
fn fs() -> Result<(), std::io::Error> {
  test_files(
    &[
      "std_lib/io/fs/readFile.tube",
      "std_lib/io/fs/removeFile.tube",
      "std_lib/io/fs/writeFile.tube",
    ],
    VmExit::Ok,
  )
}
