...
use tube_lib::create_std_lib;
use tube_vm::compiler::{Parser, Resolver};
use tube_vm::source::VM_FILE_TEST_ID;
use tube_vm::{compiler::Compiler, source::Source};
use std::fs::File;
use std::io::prelude::*;
...
fn criterion_benchmark(c: &mut Criterion) {
  ...
  let fib = load_source(
    &mut gc,
    PathBuf::from("fixture").join("criterion").join("fib.tube"),
  );
  ...
  let instantiation = load_source(
    ...
    PathBuf::from("fixture")
      .join("criterion")
      .join("instantiation.tube"),
  );
  let invocation = load_source(
    &mut gc,
    PathBuf::from("fixture")
      .join("criterion")
      .join("invocation.tube"),
  );
  let method_call = load_source(
    &mut gc,
    PathBuf::from("fixture")
      .join("criterion")
      .join("method_call.tube"),
  );
  let properties = load_source(
    &mut gc,
    PathBuf::from("fixture")
      .join("criterion")
      .join("properties.tube"),
  );
  let trees = load_source(
    &mut gc,
    PathBuf::from("fixture").join("criterion").join("trees.tube"),
  );
  let zoo = load_source(
    &mut gc,
    PathBuf::from("fixture").join("criterion").join("zoo.tube"),
  );
  let lox = load_source(
    &mut gc,
    PathBuf::from("fixture")
